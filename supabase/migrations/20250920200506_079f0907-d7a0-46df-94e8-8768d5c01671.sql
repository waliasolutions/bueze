...
-- Alter existing profiles table to add the role column and other Swiss-specific fields
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS role public.user_role DEFAULT 'homeowner';
...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS canton public.canton;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS languages TEXT[] DEFAULT ARRAY['de'];
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS verified_level INTEGER DEFAULT 0;
...
CREATE INDEX IF NOT EXISTS idx_handwerker_categories ON public.handwerker_profiles USING GIN(categories);
CREATE INDEX IF NOT EXISTS idx_handwerker_service_areas ON public.handwerker_profiles USING GIN(service_areas);
